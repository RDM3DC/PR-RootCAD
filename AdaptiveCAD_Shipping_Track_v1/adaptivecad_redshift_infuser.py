...
def gaussian_kernel_fft(nx, ny, sigma):
    import numpy as _np
    kx = _np.fft.fftfreq(nx); ky = _np.fft.fftfreq(ny)
...
def correlated_fields(nx, ny, corr=0.25, sigma=6.0, seed=0):
    ...
    u = rng.standard_normal((nx,ny)); w = rng.standard_normal((nx,ny))
    v = corr*u + _np.sqrt(max(0.0, 1.0 - corr**2))*w
    H = gaussian_kernel_fft(nx, ny, sigma)
    def blur(a): Af = _np.fft.fftn(a); return _np.fft.ifftn(Af*H).real
...
def wire_gyroid_paths(L=40.0, n_cells=3, wires_per_axis=3, amp=4.0, N=120):
    ...
    xs = np.linspace(xmin, xmax, N); ys = np.linspace(ymin, ymax, N); zs = np.linspace(zmin, zmax, N)
    k = 2*math.pi*n_cells/L
    paths = []
    for i in range(wires_per_axis):
        for j in range(wires_per_axis):
            y0 = ymin + (i+0.5)*(L/wires_per_axis); z0 = zmin + (j+0.5)*(L/wires_per_axis)
            P = np.stack([xs, y0 + np.sin(k*xs + 0.6*j)*4.0, z0 + np.cos(k*xs + 0.4*i)*4.0], axis=1); paths.append(P)
    for i in range(wires_per_axis):
        for j in range(wires_per_axis):
            x0 = xmin + (i+0.5)*(L/wires_per_axis); z0 = zmin + (j+0.5)*(L/wires_per_axis)
            P = np.stack([x0 + np.sin(k*ys + 0.5*i)*4.0, ys, z0 + np.cos(k*ys + 0.7*j)*4.0], axis=1); paths.append(P)
    for i in range(wires_per_axis):
        for j in range(wires_per_axis):
            x0 = xmin + (i+0.5)*(L/wires_per_axis); y0 = ymin + (j+0.5)*(L/wires_per_axis)
            P = np.stack([x0 + np.sin(k*zs + 0.4*j)*4.0, y0 + np.cos(k*zs + 0.6*i)*4.0, zs], axis=1); paths.append(P)
    return paths
...
def choose_rho_with_guards(lam_mean, alf_mean, params):
    def eval_rho(rho):
        ...
        return dict(rho=rho, r_wire=r_wire, t_skin=t_skin, A_eff=A_eff, density=density, tau=tau, DeltaT=dT)
    candidates = [0.25, 0.20, 0.30]
    trials = [eval_rho(r) for r in candidates]
    feas = [t for t in trials if (t['density'] <= params['density_max'] and t['tau'] <= params['tau_max'])]
    if not feas: feas = trials
...
def try_publish(topic, payload_json):
    try:
        ...
        print(payload_json); return False
    rclpy.init()
    class Pub(Node):
        def __init__(self):
...
def main():
    ...
    params = dict(t0=args.t0, r_wire0=args.r_wire0, gam_w=args.gam_w, gam_a=args.gam_a,
                  kappa=args.kappa, density_max=args.density_max, tau_max=args.tau_max)
    ...
    payload = {
        ...
        'metrics': {'A_eff': best['A_eff'], 'density': best['density'], 'tau': best['tau'], 'DeltaT': best['DeltaT']},
        'weights': {'A_eff_norm': float(A_eff_n), 't_norm': float(t_n), 'beta': args.kappa},
        'trials': trials,
    }
    js = json.dumps(payload)
    if args.publish: try_publish('/adaptivecad/redshift_config', js)
    else: print(js)

    import csv
    csv_path = outdir/"redshift_selection.csv"
    with open(csv_path, 'w', newline='') as f:
        wcsv = csv.DictWriter(f, fieldnames=['scenario','seed','rho_star','r_wire_mm','t_skin_mm','A_eff','density','tau','DeltaT'])
        wcsv.writeheader()
        wcsv.writerow({'scenario':args.scenario,'seed':args.seed,'rho_star':best['rho'],'r_wire_mm':best['r_wire'],'t_skin_mm':best['t_skin'],'A_eff':best['A_eff'],'density':best['density'],'tau':best['tau'],'DeltaT':best['DeltaT']})
    print(str(csv_path))
