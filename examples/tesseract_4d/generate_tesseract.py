...
try:
    from adaptivecad.pi.kernel import PiAParams, pi_a
...
def plot_tsp(P, order, path_png):
    ...
    a, b = order[-1], order[0]
    ax.plot(
        [P[a, 0], P[b, 0]], [P[a, 1], P[b, 1]], [P[a, 2], P[b, 2]], linewidth=1.0, linestyle=":"
    )
...
def main(out_dir="out"):
    ...
    pd.DataFrame({"order": list(range(len(tour))), "vertex": tour}).to_csv(
        Path(out_dir) / "tsp_order.csv", index=False
    )
