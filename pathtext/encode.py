...
def main():
    ...
    layout = [
        {"path": pid, "range": [0, len(text)], "spacing_px": args.spacing, "offset_px": args.offset}
    ]
    container = encode_pathtext(
        text, paths, layout, units="px", metadata={"source_text": Path(args.text).name}
    )
