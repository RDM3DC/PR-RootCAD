...
def unpack_to_svg(blob: bytes, out_svg: str, width: int=800, height: int=300, font_family: str="sans-serif") -> str:
    ...
    off = len(MAGIC)
    n, ext_len, atc_size, num_paths, num_layout = struct.unpack_from("<I H I H H", blob, off); off += struct.calcsize("<I H I H H")
    ext = blob[off:off+ext_len]; off += ext_len
    atc_data = blob[off:off+atc_size]; off += atc_size
    text_obj = {"format":"ATC-AC2-v2","n":int(n),"ext":ext.decode("utf-8"),"data_b64": base64.b64encode(atc_data).decode("ascii")}
    text = atc_unpack(text_obj)

    # paths
    paths_pts = []
    for _ in range(num_paths):
        flags, eps, count = struct.unpack_from("<H f I", blob, off); off += struct.calcsize("<H f I")
        if count == 0:
            paths_pts.append([]); continue
    ...
    for i, pts in enumerate(paths_pts):
        parts.append(f'<path id="p{i}" d="{path_d(pts)}" fill="none" stroke="#ccc" stroke-width="1"/>')
    parts.append('</defs>')
    for i, pts in enumerate(paths_pts):
    ...
    for (idx,a,b,spacing,offset) in layouts:
        ...
        parts.append(f'<text font-family="{font_family}" font-size="{spacing}px" letter-spacing="0.5px">')
