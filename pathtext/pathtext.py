...
from atc.codec_ac import pack as atc_pack
from atc.codec_ac import unpack as atc_unpack


def encode_pathtext(
    text: str, paths: List[Dict], layout: List[Dict], units: str = "px", metadata: Dict = None
) -> Dict:
    return {
        "format": "ATC-PATH-v1",
        "units": units,
...
def to_svg(
    ...
    for lay in container["layout"]:
        ...
        svg_parts.append(
            f'  <textPath href="#{pid}" startOffset="{lay.get("offset_px",0)}">{frag}</textPath>'
        )
