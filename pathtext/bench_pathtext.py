...
def main():
    ...
    run(["python","-m","pathtext.encode","--text", str(text_path), "--anchors", str(anchors_json), "--out", str(src_json)], cwd=repo)

    rows = []
    for eps in [float(x) for x in args.eps_list.split(",")]:
        # compress paths with eps
        cmp_json = repo / "pathtext" / f"bench_eps{eps}.atcp.json"
        run(["python","-m","pathtext.compress_paths", str(src_json), str(cmp_json), "--eps", str(eps)], cwd=repo)
        # binary pack
    ...
    with open(repo / args.out_csv, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["eps","n_chars","orig_points","new_points","max_err_px","bytes","bpc_bits"])
        w.writeheader()
        for r in rows:
            w.writerow(r)
