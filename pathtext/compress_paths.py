...
from .path_compress import compress_anchors, rdp
...
def main():
    ...
    for p in obj.get("paths", []):
        ...
        if args.method == "rdp":
            ...
            comp = {"method":"adaptive","eps_px": args.eps, "orig_points": len(anchors), "new_points": len(simp), "max_err_px": me, "anchors": simp}
            ...
            def max_err(orig, simp_points):
                ...
                return me
            me = max_err(anchors, simp) if len(simp)>=2 else 0.0
            comp = {"method":"topk","eps_px": 0.0, "orig_points": len(anchors), "new_points": len(simp), "max_err_px": me, "anchors": simp}
        p["anchors_cmc"] = comp
    ...
    for p in obj.get("paths", []):
        c = p.get("anchors_cmc", {})
        if c:
            print(f'Path {p.get("id")} : {c.get("orig_points")} -> {c.get("new_points")} points, max_err={c.get("max_err_px"):.2f}px ({c.get("method")})')
