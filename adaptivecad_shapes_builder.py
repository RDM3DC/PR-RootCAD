...
def cylinder_between(a, b, r=0.8, segments=14):
    ...
    if L < 1e-9:
        ...
        m = segments
        for i in range(m):
            F.append([i,(i+1)%m,m+i]); F.append([m+i,(i+1)%m,m+((i+1)%m)])
        return V, np.array(F,int)
    z = ab / L
    ref = np.array([0.0,0.0,1.0])
    if abs(np.dot(ref, z)) > 0.9: ref = np.array([1.0,0.0,0.0])
    ...
    m = segments
    for i in range(m):
        a0=i; a1=(i+1)%m; b0=i+m; b1=((i+1)%m)+m
        F.append([a0,a1,b1]); F.append([a0,b1,b0])
    # caps
    center0 = ring0.mean(axis=0); center1 = ring1.mean(axis=0)
    V = np.vstack([V, center0, center1])
    c0 = V.shape[0]-2; c1 = V.shape[0]-1
    for i in range(m):
        F.append([c0,(i+1)%m,i])
...
def hex_vertices(R=25.0, z=6.0):
    return np.array([[R*math.cos(k*math.pi/3), R*math.sin(k*math.pi/3), z] for k in range(6)], float)

def build_rt_seam_frame(R=25.0, z=6.0, r_edge=0.8, r_rib=0.65):
    V_parts=[]; F_parts=[]; base=0
    Vhex = hex_vertices(R,z)
    # ring
    for k in range(6):
        a = Vhex[k]; b = Vhex[(k+1)%6]
        Vc, Fc = cylinder_between(a,b,r=r_edge,segments=14)
        F_parts.append(Fc+base); V_parts.append(Vc); base += Vc.shape[0]
    # ribs to center
    center = np.array([0.0,0.0,z])
    for k in range(6):
        Vc, Fc = cylinder_between(Vhex[k], center, r=r_rib, segments=12)
        F_parts.append(Fc+base); V_parts.append(Vc); base += Vc.shape[0]
    V = np.vstack(V_parts); F = np.vstack(F_parts)
    return V, F

def hex_centers(R):
    a = math.sqrt(3)*R
    b = 1.5*R
    return [(0.0,0.0), ( a, 0.0), ( 0.5*a,  b), (-0.5*a,  b), (-a, 0.0), (-0.5*a,-b), ( 0.5*a,-b)]
...
def build_rt_seam_patch(R=25.0, z=6.0, r_edge=0.75, r_rib=0.6):
    ...
    for cx,cy in centers:
        Vsh = V1.copy(); Vsh[:,0]+=cx; Vsh[:,1]+=cy
        Vall.append(Vsh); Fall.append(F1+base); base+=Vsh.shape[0]
...
def tube_polyline(points, r=0.6, segments=10):
    ...
    for i in range(len(points)-1):
        ...
        F_all.append(Fa + base); V_all.append(Va); base += Va.shape[0]
    if not V_all: return np.zeros((0,3)), np.zeros((0,3), int)
...
def hopf_braid_weave(n_strands=6, p=3, q=2, R=20.0, r=6.0, r_strand=0.7, phase_jitter=0.2, seed=2):
    ...
    for s in range(n_strands):
        ...
        F_parts.append(Ft+base); V_parts.append(Vt); base += Vt.shape[0]
...
def wire_gyroid_weave(L=40.0, n_cells=3, wires_per_axis=3, amp=4.0, r_wire=0.6, N=120):
    ...
    for P in paths:
        ...
        F_all.append(Ft+base); V_all.append(Vt); base += Vt.shape[0]
    return np.vstack(V_all), np.vstack(F_all)
...
def synth_flux(X, Y, R):
    ...
    q *= m
    if np.any(m): q = (q - q[m].min())/(q[m].ptp()+1e-12)
    return q

def triangulate_skin(xs, ys, mask, tmap, base_z):
    nx, ny = len(xs), len(ys)
    def vid(i,j,top): return (i*ny + j)*2 + (1 if top else 0)
    ...
    for i in range(nx-1):
        for j in range(ny-1):
            ...
            if cell_inside:
                a=vid(i,j,True); b=vid(i+1,j,True); c=vid(i+1,j+1,True); d=vid(i,j+1,True)
                faces += [[a,b,c],[a,c,d]]
                a=vid(i,j,False); b=vid(i+1,j,False); c=vid(i+1,j+1,False); d=vid(i,j+1,False)
                faces += [[c,b,a],[d,c,a]]
    for i in range(nx-1):
        for j in range(ny):
            a_inside = mask[i,j]; b_inside = mask[i+1,j]
            if a_inside != b_inside:
                if a_inside: left=(i,j); right=(i+1,j)
                else: left=(i+1,j); right=(i,j)
                a0=vid(left[0], left[1], False); a1=vid(left[0], left[1], True)
                b0=vid(right[0], right[1], False); b1=vid(right[0], right[1], True)
                faces += [[a0,b0,b1],[a0,b1,a1]]
    for i in range(nx):
        for j in range(ny-1):
            a_inside = mask[i,j]; b_inside = mask[i,j+1]
            if a_inside != b_inside:
                if a_inside: low=(i,j); high=(i,j+1)
                else: low=(i,j+1); high=(i,j)
                a0=vid(low[0], low[1], False); a1=vid(low[0], low[1], True)
                b0=vid(high[0], high[1], False); b1=vid(high[0], high[1], True)
                faces += [[a0,b0,b1],[a0,b1,a1]]
...
def main():
    ...
    ap.add_argument("--shape", required=True, choices=[
        "rt_seam_hex","rt_seam_patch","hopf_weave","wire_weave","ablative_skin"
    ])
    ...
    if shape == "rt_seam_hex":
        ...
        V,F = hopf_braid_weave(n_strands=args.n_strands, p=args.p, q=args.q, r_strand=args.r_strand)
