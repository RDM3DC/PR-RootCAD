...
def parallel_transport_frame(C):
    ...
    Nvec = np.zeros_like(C)
    Bvec = np.zeros_like(C)
    Nvec[0] = n0
    Bvec[0] = np.cross(T[0], Nvec[0])
...
def run_once(args):
    ...
    trC = make_hotspots(
        t, args.hotspots, neg_frac=(args.neg_frac if args.neg_energy else 0.0), seed=args.seed
    )
    verts, faces, f = build_ribbon(
        C, Nvec, Bvec, args.lam, trC, w=args.width, M=args.M, twist=args.twist
    )
    # color by π_a
    pi = math.pi
    pi_a = pi * (1 + args.lam * trC)
    col = np.repeat(pi_a, args.M)
    col = (col - col.min()) / (col.max() - col.min() + 1e-12)
    # metrics
    ...
    with open(out / "metrics.csv", "w", newline="") as fcsv:
        w = csv.writer(fcsv)
        w.writerow(["n_throat", "min_width", "mean_width", "lambda", "hotspots", "neg_energy"])
        w.writerow([n_throat, minw, meanw, args.lam, args.hotspots, int(args.neg_energy)])
...
def sweep_lambda(args):
    ...
    fig.tight_layout()
    fig.savefig(out / "lambda_persistence.png", dpi=170)
    plt.close(fig)
...
def main():
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.25,
        help="fraction of hotspots that are negative when --neg_energy on",
    )
