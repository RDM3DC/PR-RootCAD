...
try:
    ...
    class SketchEntity:
        ...
        def snap_points(self):
            return []  # list of (x,y) in world (Z=0)
    ...
    class Polyline2D(SketchEntity):
        ...
        def draw(self, p: QPainter, view):
            ...
            if not self.pts:
                return
            pts2 = [
                view._world_to_screen(_np.array([x, y, 0.0], _np.float32)) for (x, y) in self.pts
            ]
            if len(pts2) >= 2:
                pen = QPen(QColor(220, 220, 220))
                pen.setWidth(2)
                if getattr(self, "construction", False):
    ...
    class Circle2D(SketchEntity):
        ...
        def snap_points(self):
            ...
            if self.radius > 1e-6:
                for ang in (0, 90, 180, 270):
                    a = _m.radians(ang)
                    pts.append(
                        self.center + self.radius * _np.array([_np.cos(a), _np.sin(a)], _np.float32)
                    )
            return pts
        ...
        def draw(self, p: QPainter, view):
            ...
            pen = QPen(QColor(255, 180, 120))
            pen.setWidth(6)
            p.setPen(pen)
            p.drawPoint(int(c[0]), int(c[1]))
    ...
    class Rect2D(SketchEntity):
        ...
        def corners(self):
            ...
            return [
                _np.array([mnx, mny], _np.float32),
                _np.array([mxx, mny], _np.float32),
                _np.array([mxx, mxy], _np.float32),
                _np.array([mnx, mxy], _np.float32),
            ]
    ...
    class Arc2D(SketchEntity):
        ...
        def draw(self, p: QPainter, view):
            ...
            if self.p1 is None or self.p2 is None:
                ...
                p.drawPoint(int(s0[0]), int(s0[1]))
                if self.p1 is not None:
                    s1 = view._world_to_screen(
                        _np.array([self.p1[0], self.p1[1], 0.0], _np.float32)
                    )
                    p.drawLine(int(s0[0]), int(s0[1]), int(s1[0]), int(s1[1]))
                return
            circ = self._circle_from_3pts()
            ...
            for a, b in zip(pts[:-1], pts[1:]):
                p.drawLine(int(a[0]), int(a[1]), int(b[0]), int(b[1]))
            pen_pts = QPen(QColor(255, 200, 140))
            pen_pts.setWidth(6)
            p.setPen(pen_pts)
            s0 = view._world_to_screen(_np.array([self.p0[0], self.p0[1], 0.0], _np.float32))
            p.drawPoint(int(s0[0]), int(s0[1]))
            s2 = view._world_to_screen(_np.array([self.p2[0], self.p2[1], 0.0], _np.float32))
            p.drawPoint(int(s2[0]), int(s2[1]))
            sc = view._world_to_screen(_np.array([c[0], c[1], 0.0], _np.float32))
            p.drawPoint(int(sc[0]), int(sc[1]))
    ...
    class Arc2D(SketchEntity):
        ...
        def snap_points_typed(self):
            out = []
            if self.p0 is not None:
                out.append((self.p0, "end"))
            if self.p2 is not None:
                out.append((self.p2, "end"))
            circ = self._circle_from_3pts()
            if circ is not None:
                c, _ = circ
                out.append((c, "center"))
    ...
    class SketchLayer:
        ...
        def draw(self, p: QPainter, view):
            ...
            if self.hover_snap is not None:
                s = view._world_to_screen(
                    _np.array([self.hover_snap[0], self.hover_snap[1], 0.0], _np.float32)
                )
                pen = QPen(QColor(120, 255, 160))
                pen.setWidth(2)
                p.setPen(pen)
    ...
    class DimLinear2D(DimBase2D):
        ...
        def _xy(self, ref):
            ...
            try:
                ...
                if (
                    hasattr(ref, "shape")
                    and getattr(ref, "shape", None) is not None
                    and len(ref.shape) == 1
                    and ref.shape[0] == 2
                ):
                    return ref
        ...
        def draw(self, p: QPainter, view):
            ...
            a = self._xy(self.ref_a)
            b = self._xy(self.ref_b)
    ...
    class DimRadius2D(DimBase2D):
        def __init__(self, circle_ref, panel):
            super().__init__()
            self.circle_ref = circle_ref
            self.panel = panel

        def draw(self, p: QPainter, view):
            if not (
                isinstance(self.circle_ref, tuple)
                and self.circle_ref[0] == "circle"
                and isinstance(self.circle_ref[1], Circle2D)
            ):
                return
            c = self.circle_ref[1]
            val = float(c.radius)
...
def _compute_world_tolerance(
    view, center: np.ndarray, pixels: float, fov_deg: float
    ...
    class DimLinear2D(DimBase2D):
        ...
        def _xy(self, ref):
            ...
            if isinstance(ref, tuple) and len(ref) == 2 and isinstance(ref[0], Line2D):
                ...
                if idx == 0:
                    return ent.p0
                if idx == 1 and ent.p1 is not None:
                    return ent.p1
                return None
            if isinstance(ref, tuple) and len(ref) == 2 and isinstance(ref[0], Rect2D):
                ent, idx = ref
...
class AnalyticViewport(QOpenGLWidget):
    def __init__(self, parent=None, aacore_scene: AACoreScene | None = None):
        ...
        self._sk_point_sel = (
            None  # (entity, index) for Polyline2D point or ('circle', entity) for circle center
        )
    ...
    def initializeGL(self):
        ...
        glShaderSource(vsh, vs)
        glCompileShader(vsh)
        if not glGetShaderiv(vsh, GL_COMPILE_STATUS):
            raise RuntimeError(glGetShaderInfoLog(vsh).decode())
        fsh = glCreateShader(GL_FRAGMENT_SHADER)
        glShaderSource(fsh, fs)
        glCompileShader(fsh)
        if not glGetShaderiv(fsh, GL_COMPILE_STATUS):
            raise RuntimeError(glGetShaderInfoLog(fsh).decode())
        glAttachShader(self.prog, vsh)
        glAttachShader(self.prog, fsh)
        ...
        try:
            ...
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, self._pick_tex_micro, 0
            )
    ...
    def _init_picking(self):
        try:
            ...
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST)
            glFramebufferTexture2D(
                GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, self._pick_tex, 0
            )
            self._depth_rb = glGenRenderbuffers(1)
            glBindRenderbuffer(GL_RENDERBUFFER, self._depth_rb)
            glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, self.width(), self.height())
            glFramebufferRenderbuffer(
                GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, self._depth_rb
            )
        ...
        if self._poly_overlays:
            ...
            for ov in self._poly_overlays:
                try:
                    ...
                    pen = QPen(QColor(int(col[0]), int(col[1]), int(col[2])))
                    pen.setWidth(width)
                    p.setPen(pen)
    ...
    def _draw_grid(self, painter: QPainter, panel):
        ...
        pen = QPen(QColor(int(cr), int(cg), int(cb), int(ca)))
        pen.setWidth(1)
        painter.setPen(pen)
        # verticals
        x = x0
    ...
    def _render_scene_internal(self, debug_override=None):
        ...
        R = self._cam_basis()
        glUniformMatrix3fv(glGetUniformLocation(self.prog, "u_cam_rot"), 1, GL_FALSE, R.T)
        glUniform2f(
            glGetUniformLocation(self.prog, "u_res"), float(self.width()), float(self.height())
        )
    ...
    def keyPressEvent(self, event):
        ...
        if k in (Qt.Key_0, Qt.Key.Key_0):
            self.debug_mode = 0
            changed = True
        elif k in (Qt.Key_1, Qt.Key.Key_1):
            self.debug_mode = 1
            changed = True
        elif k in (Qt.Key_2, Qt.Key.Key_2):
            self.debug_mode = 2
            changed = True
        elif k in (Qt.Key_3, Qt.Key.Key_3):
            self.debug_mode = 3
            changed = True
        elif k in (Qt.Key_4, Qt.Key.Key_4):
            self.debug_mode = 4
            changed = True
        elif k in (Qt.Key_5, Qt.Key.Key_5):
            self.debug_mode = 5
            changed = True  # adaptive heatmap
        elif k == Qt.Key_Space:
            self.debug_mode = (self.debug_mode + 1) % 6
            changed = True
        elif k == Qt.Key_A:
            self.use_analytic_aa ^= 1
            changed = True
            ...
            self.toon_levels = min(self.toon_levels + 1.0, 8.0)
            changed = True
        elif k == Qt.Key_Minus:
            self.toon_levels = max(self.toon_levels - 1.0, 1.0)
            changed = True
        elif k == Qt.Key_C:
            # cycle curvature strength
            if self.curv_strength < 0.25:
                self.curv_strength = 0.5
            elif self.curv_strength < 0.75:
                self.curv_strength = 1.0
            else:
                self.curv_strength = 0.0
            changed = True
        elif k == Qt.Key_G:
            self.save_gbuffers()
            return
            ...
            self.show_beta_overlay ^= 1
            changed = True
        elif k == Qt.Key_Period:  # increase beta overlay intensity
            self.beta_overlay_intensity = min(1.0, self.beta_overlay_intensity + 0.05)
            changed = True
        elif k == Qt.Key_Comma:  # decrease intensity
            self.beta_overlay_intensity = max(0.0, self.beta_overlay_intensity - 0.05)
            changed = True
        elif k == Qt.Key_Slash:  # increase beta scale
            self.beta_scale = min(8.0, self.beta_scale * 1.25)
            changed = True
        elif k == Qt.Key_Question:  # SHIFT + '/' may map same; ensure alternative
            self.beta_scale = min(8.0, self.beta_scale * 1.25)
            changed = True
        elif k == Qt.Key_Backslash:  # decrease beta scale
            self.beta_scale = max(0.125, self.beta_scale / 1.25)
            changed = True
        elif k == Qt.Key_V:  # cycle beta color map
            self.beta_cmap = (self.beta_cmap + 1) % 4
            changed = True
        if changed:
            self._update_title()
            self.update()
            ...
            if (
                panel is not None
                and getattr(panel, "_sketch_on", False)
                and panel._sketch_mode == "polyline"
            ):
            ...
            if panel is not None and getattr(panel, "_sketch_on", False) and k == Qt.Key_Escape:
                try:
                    panel._sketch._dim_placing = False
                    panel._sketch._dim_place_data = None
            ...
            if (
                panel is not None
                and getattr(panel, "_sketch_on", False)
                and k in (Qt.Key_Delete, Qt.Key_Backspace)
            ):
            ...
            if panel is not None:
                if k == Qt.Key_R:
                    panel._gizmo_mode = "rotate"
                    self.update()
                    return
                elif k == Qt.Key_S and not (
                    event.modifiers() & Qt.KeyboardModifier.ControlModifier
                ):
                    panel._gizmo_mode = "scale"
                    self.update()
                    return
                elif k == Qt.Key_M:
                    panel._gizmo_mode = "move"
                    self.update()
                    return
                elif k == Qt.Key_X:
                    panel._axis_lock = "x"
                    ...
                    panel._axis_lock = "z"
                    return
                elif k == Qt.Key_Escape and not (
                    getattr(panel, "_sketch_on", False) and panel._sketch_mode == "polyline"
                ):
                    panel._axis_lock = None
                    return
            ...
            if pr is not None:
                ...
                if k == Qt.Key_W:
                    pos_vec += basis[:, 1] * step
                    moved = True
                elif k == Qt.Key_S:
                    pos_vec -= basis[:, 1] * step
                    moved = True
                elif k == Qt.Key_D:
                    pos_vec += basis[:, 0] * step
                    moved = True
                elif k == Qt.Key_A:
                    pos_vec -= basis[:, 0] * step
                    moved = True
                elif k == Qt.Key_E:
                    pos_vec -= basis[:, 2] * step
                    moved = True  # forward (-Z in basis third column sign depending)
                elif k == Qt.Key_Q:
                    pos_vec += basis[:, 2] * step
                    moved = True
                # World-aligned (arrows + PgUp/PgDn)
                elif k == Qt.Key_Up:
                    pos_vec += np.array([0, step, 0], np.float32)
                    moved = True
                elif k == Qt.Key_Down:
                    pos_vec -= np.array([0, step, 0], np.float32)
                    moved = True
                elif k == Qt.Key_Right:
                    pos_vec += np.array([step, 0, 0], np.float32)
                    moved = True
                elif k == Qt.Key_Left:
                    pos_vec -= np.array([step, 0, 0], np.float32)
                    moved = True
                elif k == Qt.Key_PageUp:
                    pos_vec += np.array([0, 0, step], np.float32)
                    moved = True
    ...
    def add_polyline_overlay(
        self, pts, color=(120, 255, 160), width=3, scale=(1.0, 1.0), offset=(0.0, 0.0)
    ):
        try:
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if (
            ...
            and self._current_prim() is not None
            and (move_shift or move_ctrl or gizmo_move)
        ):
        ...
        if (
            ...
            if getattr(panel, "_sketch_snap_grid", False):
                ...
                w3 = self._screen_to_world_grid_snap(
                    event.position().x(), event.position().y(), step
                )
            else:
            ...
            if mode == "polyline":
                ...
                panel._sketch.active_poly.pts.append(
                    np.array([pt_world2[0], pt_world2[1]], np.float32)
                )
                self.update()
                return
            elif mode == "circle":
                if panel._sketch.active_shape is None:
                    ...
                    # finalize
                    if (
                        isinstance(panel._sketch.active_shape, Circle2D)
                        and panel._sketch.active_shape.radius > 0
                    ):
                        ...
                        try:
                            if bool(getattr(panel, "_sketch_circle_to_ring", False)):
                                ...
                                r = max(0.01, R * 0.05)
                                pr = Prim(
                                    KIND_TORUS, [R, r, 0.0, 0.0], beta=0.0, color=(0.9, 0.7, 0.3)
                                )
                                if hasattr(pr, "set_transform"):
                                    pr.set_transform(
                                        pos=[float(c.center[0]), float(c.center[1]), 0.0],
                                        euler=[0.0, 0.0, 0.0],
                                        scale=[1.0, 1.0, 1.0],
                                    )
                                else:
                                    pr.xform.M[:3, 3] = np.array(
                                        [float(c.center[0]), float(c.center[1]), 0.0], np.float32
                                    )
                                self.scene.add(pr)
                ...
                if panel._sketch.active_shape is None:
                    ...
                    if isinstance(panel._sketch.active_shape, Rect2D):
                        panel._sketch.active_shape.p1 = np.array(
                            [pt_world2[0], pt_world2[1]], np.float32
                        )
                ...
                if panel._sketch.active_shape is None:
                    ...
                    isinstance(panel._sketch.active_shape, Arc2D)
                    and panel._sketch.active_shape.p1 is None
                ):
                    panel._sketch.active_shape.p1 = np.array(
                        [pt_world2[0], pt_world2[1]], np.float32
                    )
                    ...
                    if isinstance(panel._sketch.active_shape, Arc2D):
                        panel._sketch.active_shape.p2 = np.array(
                            [pt_world2[0], pt_world2[1]], np.float32
                        )
                        panel._sketch.entities.append(panel._sketch.active_shape)
                    panel._sketch.active_shape = None
                ...
                if panel._sketch._dim_placing and isinstance(panel._sketch._dim_place_data, dict):
                    ...
                    a = panel._sketch._dim_place_data.get("a")
                    b = panel._sketch._dim_place_data.get("b")
                ...
                if dt in ("radius", "diameter"):
                    if (
                        ...
                        and sel[0] == "circle"
                        and isinstance(sel[1], Circle2D)
                    ):
                        if dt == "radius":
                            panel._sketch.dimensions.append(DimRadius2D(("circle", sel[1]), panel))
                        else:
                            panel._sketch.dimensions.append(
                                DimDiameter2D(("circle", sel[1]), panel)
                            )
                        self.update()
                        return
                    else:
                ...
                try:
                    ...
                    for ent in panel._sketch.entities:
                        if isinstance(ent, Polyline2D) and len(ent.pts) >= 2:
                            for i in range(len(ent.pts) - 1):
                                ...
                                dpx = float(
                                    np.hypot(
                                        s[0] - event.position().x(), s[1] - event.position().y()
                                    )
                                )
                                if dpx < best_px:
                                    best_px = dpx
                            ...
                            s = self._world_to_screen(np.array([p[0], p[1], 0.0], np.float32))
                            dpx = float(
                                np.hypot(s[0] - event.position().x(), s[1] - event.position().y())
                            )
                            if dpx < best_px:
                                best_px = dpx
                                best_ent = ent
                    ...
                    if isinstance(best_ent, Polyline2D):
                        ...
                        b = best_ent.p1
                        mid = (
                            best_point if getattr(panel, "_insert_midpoint", False) else w2
                        ).copy()
    ...
    def mouseMoveEvent(self, event: QMouseEvent):
        ...
        if panel is not None and getattr(panel, "_sketch_on", False):
            ...
            for sp in panel._sketch.all_snap_points():
                ...
                dpx = float(
                    np.hypot(scr_sp[0] - event.position().x(), scr_sp[1] - event.position().y())
                )
            ...
            if panel._sketch_mode == "dimension":
                ...
                if panel._sketch._dim_placing and isinstance(panel._sketch._dim_place_data, dict):
                    ...
                    sel = self._pick_nearest_sketch_vertex(
                    ...
                    if dt in ("radius", "diameter"):
                        if (
                            ...
                            if dt == "radius":
                                ...
                                panel._sketch._dim_preview = DimDiameter2D(
                                    ("circle", sel[1]), panel
                                )
                    else:
                        a = panel._sketch._dim_pick
                        b = (
                            ...
                            if sel is not None
                                ...
                                panel._sketch.hover_snap
                                if panel._sketch.hover_snap is not None
                                else wpt
                            )
                        )
            ...
            if (
                self._sk_dragging
                and self._sk_point_sel is not None
                and (event.buttons() & Qt.MouseButton.LeftButton)
            ):
                if getattr(panel, "_sketch_snap_grid", False):
                    step = self._grid_step_world_for_panel(panel)
        ...
        if (
            panel is not None
            and hasattr(panel, "_gizmo_mode")
            and panel._gizmo_mode in ("rotate", "scale")
            and panel._current_sel >= 0
        ):
            if event.buttons() & Qt.MouseButton.LeftButton:  # require LMB drag
                ...
                if panel._gizmo_mode == "rotate":
                    ...
                    if local_space:
                        ...
                        T[:3, 3] = pr.xform.M[:3, 3]
                        Tinv = np.eye(4, dtype=np.float32)
                        Tinv[:3, 3] = -pr.xform.M[:3, 3]
                    ...
                    try:
                        ...
                        pass
                    try:
                        panel._select_prim(panel._current_sel)
                    except Exception:
                        pass
                    self.update()
                    self._last_mouse = event.position()
        ...
        try:
            panel = self.parent()
            lock_top = bool(
                getattr(panel, "_sketch_on", False) and getattr(panel, "_sketch_lock_top", False)
            )
        except Exception:
            lock_top = False
            ...
            if isinstance(ent, Polyline2D):
                ...
                if len(corners) >= 2:
                    for edge_idx, (a, b) in enumerate(zip(corners, corners[1:] + corners[:1])):
                        ...
                        if d < best and d <= 12.0:
                            best = d
                            best_key = (ent, ("edge_mid", edge_idx))
                if len(corners) == 4:
                ...
                try:
                    ...
                    if hasattr(mw, "statusBar") and mw.statusBar():
                        mw.statusBar().showMessage(
                            f"Moved prim {getattr(panel,'_current_sel',-1)} to ("
                            + ", ".join(f"{panel._sp_move[i].value():.3f}" for i in range(3))
                            + ")",
                            1500,
                        )
                except Exception:
    ...
    def _perform_pick(self, x: int, y: int):
        ...
        try:
            ...
            self.update()
            log.debug(
                f"pick raw={px[0,0].tolist()} enc={enc} sel={self.selected_index} at ({x},{y}) dpr={dpr}"
            )
        except Exception as e:
    ...
    def _update_title(self):
        try:
            ...
            m = (
                mode_names[self.debug_mode]
                if 0 <= self.debug_mode < len(mode_names)
                else str(self.debug_mode)
            )
...
class AnalyticViewportPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        # --- Debug Modes ---
        gb_modes = QGroupBox("Debug / Modes")
        fm = QFormLayout()
        gb_modes.setLayout(fm)
        self.mode_box = QComboBox()
        self.mode_box.addItems(["Beauty", "Normals", "ID", "Depth", "Thickness"])
        self.mode_box.currentIndexChanged.connect(self._on_mode)
        fm.addRow("Mode", self.mode_box)
        side.addWidget(gb_modes)
        self._register_section("debug_modes", "Debug", gb_modes)

        # --- Feature Toggles ---
        gb_feat = QGroupBox("Features")
        vf = QVBoxLayout()
        gb_feat.setLayout(vf)
        self.cb_aa = QCheckBox("Analytic AA")
        self.cb_aa.setChecked(True)
        self.cb_aa.stateChanged.connect(self._on_toggle)
        self.cb_fov = QCheckBox("Foveated Steps")
        self.cb_fov.setChecked(True)
        self.cb_fov.stateChanged.connect(self._on_toggle)
        self.cb_toon = QCheckBox("Toon")
        self.cb_toon.setChecked(False)
        self.cb_toon.stateChanged.connect(self._on_toggle)
        for w in (self.cb_aa, self.cb_fov, self.cb_toon):
            vf.addWidget(w)
        side.addWidget(gb_feat)
        self._register_section("features", "Features", gb_feat)

        # --- Parameters ---
        gb_sliders = QGroupBox("Parameters")
        fs = QFormLayout()
        gb_sliders.setLayout(fs)
        self.slider_toon = QSlider(Qt.Orientation.Horizontal)
        self.slider_toon.setRange(1, 8)
        self.slider_toon.setValue(4)
        self.slider_toon.valueChanged.connect(self._on_toon_levels)
        self.slider_curv = QSlider(Qt.Orientation.Horizontal)
        self.slider_curv.setRange(0, 100)
        self.slider_curv.setValue(100)
        self.slider_curv.valueChanged.connect(self._on_curv)
        self.cb_beta = QCheckBox("Î² Overlay")
        self.cb_beta.setChecked(False)
        self.cb_beta.stateChanged.connect(self._on_toggle)
        self.slider_beta_int = QSlider(Qt.Orientation.Horizontal)
        self.slider_beta_int.setRange(0, 100)
        self.slider_beta_int.setValue(int(self.view.beta_overlay_intensity * 100))
        self.slider_beta_int.valueChanged.connect(self._on_beta_int)
        self.slider_beta_scale = QSlider(Qt.Orientation.Horizontal)
        self.slider_beta_scale.setRange(1, 800)
        self.slider_beta_scale.setValue(int(self.view.beta_scale * 100))
        self.slider_beta_scale.valueChanged.connect(self._on_beta_scale)
        self.cmap_box = QComboBox()
        self.cmap_box.addItems(["Legacy", "Viridis", "Plasma", "Diverge"])
        self.cmap_box.currentIndexChanged.connect(self._on_cmap)
        ...
        if _HAVE_WHEEL:
            ...
            self._slider_wheel_thickness, self._label_wheel_thickness, row_thickness = _make_slider(
                6, 40, thickness_val, lambda v: f"{v}%"
            )
            ...
            self._slider_wheel_opacity, self._label_wheel_opacity, row_opacity = _make_slider(
                0, 100, opacity_val, lambda v: f"{v}%"
            )
            ...
            self._slider_wheel_text, self._label_wheel_text, row_text = _make_slider(
                60, 200, text_val, lambda v: f"{v/100:.2f}x"
            )
            ...
            self._slider_wheel_spin, self._label_wheel_spin, row_spin = _make_slider(
                0, 360, spin_val, lambda v: f"{v} deg/s"
            )
    ...
    def _add_tesseract(self):
        ...
        if len(self.view.scene.prims) + need > MAX_PRIMS:
            log.info(
                f"[tesseract] Not enough prim budget: need {need}, have {MAX_PRIMS-len(self.view.scene.prims)}"
            )
            return
        # Create prims
        ...
        # Initial angles
        angs = {
            "xy": float(self._tess_ang_xy.value()),
            "xz": float(self._tess_ang_xz.value()),
            "xw": float(self._tess_ang_xw.value()),
            "yz": float(self._tess_ang_yz.value()),
            "yw": float(self._tess_ang_yw.value()),
            "zw": float(self._tess_ang_zw.value()),
        }
        pidx_edges = []
        for i, j in edges:
        ...
        rig = {
            ...
            "edges": edges,
            "verts4": verts4,
            "pidx_edges": pidx_edges,
            "pidx_nodes": pidx_nodes,
            "edge_r": er,
            "node_r": nr,
            "L": L,
            "proj": proj,
            "fw": fw,
            "angs": angs,
            "autospin": bool(self._tess_autospin.isChecked()),
            "speed": float(self._tess_speed.value()),
        }
        ...
        for rig in self._rigs:
            try:
                if rig.get("type") == "tesseract":
                    ...
                    if rig.get("autospin", False):
                        rig["angle"] = (
                            float(rig.get("angle", 0.0)) + float(rig.get("speed", 0.0)) * dt
                        )
                        ...
                        if t < dur:
                            ...
                            for pidx in rig.get("pidx_atoms", []):
                                if pidx < len(self.view.scene.prims):
                                    ...
                                    base_r = (
                                        pr.params[0] / (current_atom_scale / target_atom)
                                        if target_atom > 0
                                        else 0.5
                                    )
    ...
    def _update_tesseract_rig(self, rig):
        ...
        for eidx, (i, j) in enumerate(rig["edges"]):
            ...
            tmp = (
                ...
                if abs(float(y[0])) < 0.9
                else np.array([0.0, 0.0, 1.0], np.float32)
            )
            x = np.cross(tmp, y)
            x /= max(1e-6, np.linalg.norm(x))
            z = np.cross(y, x)
            ...
            pr.params[0] = float(rig["edge_r"])
            pr.params[1] = float(h)
            pr.xform.M = M
            try:
                pr.xform.M_inv = np.linalg.inv(M)
            except Exception:
                pr.xform.M_inv = None
        # Update nodes
        if rig["pidx_nodes"]:
    ...
    def _build_scale_sphere_rig(self):
        try:
            ...
            base_radius = max(
                1e-5,
                0.5
                * (2.0 * depth_now * math.tan(math.radians(fov_deg) * 0.5) / max(1.0, width_px)),
            )
            ...
            for spec in feature_specs:
                ...
                pr.set_transform(
                    pos=center + spec["offset"], euler=(0.0, 0.0, 0.0), scale=(1.0, 1.0, 1.0)
                )
                scene.add(pr)
                feat = {
                    "level": spec["level"],
    ...
    def _start_scale_sphere_journey(self):
        try:
            ...
            rig["d0"] = max(
                _distance_for_epsilon(start_eps, width_px, pixels, fov_deg),
                rig["base_radius"] * 40.0,
            )
            rig["d1"] = max(
                0.02,
                min(
                    _distance_for_epsilon(end_eps, width_px, pixels, fov_deg),
                    rig["base_radius"] * 0.2,
                ),
            )
            if rig["d0"] <= rig["d1"]:
    ...
    def _stop_scale_sphere(self):
        try:
            ...
            if 0 <= base_idx < len(self.view.scene.prims):
                self.view.scene.prims[base_idx].params[0] = float(
                    rig.get("base_radius", self.view.scene.prims[base_idx].params[0])
                )
            try:
                self.view.scene._notify()
            except Exception:
    ...
    def _update_scale_sphere_rig(self, rig, dt):
        try:
            ...
            if rig.get("running"):
                ...
                d = (
                    rig.get("d0", self.view.distance) * (1.0 - eased)
                    + rig.get("d1", rig.get("d0", self.view.distance) * 0.5) * eased
                )
            ...
            for feat in rig.get("features", []):
                ...
                if "color_a" in feat and "color_b" in feat:
                    col = tuple(
                        feat["color_a"][i] * (1.0 - w) + feat["color_b"][i] * w for i in range(3)
                    )
                    pr.color[:] = col
                    refreshed = True
            if refreshed:
            ...
            if status is not None:
                ...
                if nxt:
                    status.setText(
                        f"{level.replace('_', ' ').title()} -> {nxt.replace('_', ' ').title()} ({blend * 100.0:.0f}%) | eps~{eps_text}"
                    )
                else:
    ...
    def _import_xyz_molecule(self):
        try:
            ...
            path, _ = QFileDialog.getOpenFileName(
                self, "Import Molecule (XYZ)", filter="XYZ (*.xyz)"
            )
            if not path:
                return
    ...
    def _build_molecule_rig(
        self, atoms, atom_scale=0.6, bond_r=0.035, bond_thresh=0.2, autospin=True, speed=25.0
    ):
        try:
            ...
            if len(self.view.scene.prims) + need > MAX_PRIMS:
                ...
                QMessageBox.warning(
                    self,
                    "Import XYZ",
                    f"Not enough primitive budget for molecule: need {need}, have {MAX_PRIMS-len(self.view.scene.prims)}",
                )
                return None
            pidx_atoms = []
            for i, a in enumerate(atoms):
                ...
                pr = Prim(
                    ...
                    [r, 0, 0, 0],
                    beta=0.0,
                    color=tuple(a.get("color", (0.7, 0.7, 0.7))),
                )
                M = np.eye(4, dtype=np.float32)
                M[:3, 3] = pts[i]
                pr.xform.M = M
            ...
            for i, j in bonds:
                pr = Prim(
                    KIND_CAPSULE, [float(bond_r), 1.0, 0.0, 0.0], beta=0.0, color=(0.85, 0.85, 0.85)
                )
                self.view.scene.add(pr)
    ...
    def _update_molecule_rig(self, rig):
        try:
            ...
            ca = np.cos(np.deg2rad(ang))
            sa = np.sin(np.deg2rad(ang))
    ...
    def _update_sketch_info(self, sel_key):
        try:
            if sel_key is None:
                self._sketch_info_label.setText("No selection")
                return
            if (
                isinstance(sel_key, tuple)
                and len(sel_key) == 2
                and isinstance(sel_key[0], Polyline2D)
            ):
                ent, idx = sel_key
                self._sketch_info_label.setText(
                    f"Polyline: {len(getattr(ent,'pts',[]))} pts, vertex #{idx}"
                )
                return
            if (
                isinstance(sel_key, tuple)
                and sel_key[0] == "circle"
                and isinstance(sel_key[1], Circle2D)
            ):
                ent = sel_key[1]
    ...
    def _register_section(
        self, key: str, label: str, widget: QWidget, default_visible: bool = True
    ) -> None:
        if not hasattr(self, "_section_vis") or not isinstance(self._section_vis, dict):
            self._section_vis = {}
    ...
    def _populate_side_panel_controls(self, side):
        ...
        gb_gizmo = QGroupBox("Gizmo / Transform")
        fg = QFormLayout()
        gb_gizmo.setLayout(fg)
        # Mode buttons
        row_modes = QWidget()
        hm = QHBoxLayout(row_modes)
        hm.setContentsMargins(0, 0, 0, 0)
        btn_move = QPushButton("Move")
        btn_rot = QPushButton("Rotate [R]")
        btn_scl = QPushButton("Scale [S]")
        for b in (btn_move, btn_rot, btn_scl):
            b.setCheckable(True)
        btn_move.setChecked(True)

        def _set_mode(m):
            def f():
                btn_move.setChecked(m == "move")
                btn_rot.setChecked(m == "rotate")
                btn_scl.setChecked(m == "scale")
                self._gizmo_mode = m
        ...
        hm.addWidget(btn_move)
        hm.addWidget(btn_rot)
        hm.addWidget(btn_scl)
        fg.addRow("Mode", row_modes)
        # Axis lock buttons
        row_axes = QWidget()
        ha = QHBoxLayout(row_axes)
        ha.setContentsMargins(0, 0, 0, 0)
        ax_x = QPushButton("X")
        ax_y = QPushButton("Y")
        ax_z = QPushButton("Z")
        ax_clr = QPushButton("Free")
        for b in (ax_x, ax_y, ax_z, ax_clr):
            b.setCheckable(True)
        ...
        # Space toggle
        cb_space = QCheckBox("Local Space")
        cb_space.setChecked(True)

        def _space_toggle(_v):
            self._space_local = cb_space.isChecked()

        cb_space.stateChanged.connect(_space_toggle)
        fg.addRow("Space", cb_space)
        # Snap controls
        spin_ang = QDoubleSpinBox()
        spin_ang.setRange(0.5, 45.0)
        spin_ang.setSingleStep(0.5)
        spin_ang.setValue(self._snap_angle_deg)
        spin_ang.setSuffix(" Ã‚Â°")
        spin_scl = QDoubleSpinBox()
        spin_scl.setRange(0.01, 10.0)
        spin_scl.setDecimals(3)
        spin_scl.setSingleStep(0.05)
        spin_scl.setValue(self._snap_scale_step)

        def _upd_ang(_v):
            self._snap_angle_deg = float(spin_ang.value())

        def _upd_scl(_v):
            self._snap_scale_step = float(spin_scl.value())
        ...
        # --- Sketch (2D) overlay ---
        gb_sk = QGroupBox("Sketch (2D)")
        fs = QFormLayout()
        gb_sk.setLayout(fs)
        cb_sketch = QCheckBox("Enable Sketch Overlay")
        cb_sketch.setChecked(False)
        ...
        row_lock = QWidget()
        hlk = QHBoxLayout(row_lock)
        hlk.setContentsMargins(0, 0, 0, 0)
        cb_lock_top = QCheckBox("Lock Top View (XY)")
        cb_lock_top.setChecked(True)
        ...
        row_grid = QWidget()
        hg = QHBoxLayout(row_grid)
        hg.setContentsMargins(0, 0, 0, 0)
        cb_grid = QCheckBox("Show Grid")
        cb_grid.stateChanged.connect(
            lambda _v: setattr(self, "_sketch_grid_on", cb_grid.isChecked()) or self.view.update()
        )
        cb_snapg = QCheckBox("Snap Grid")
        cb_snapg.stateChanged.connect(
            lambda _v: setattr(self, "_sketch_snap_grid", cb_snapg.isChecked())
        )
        # Toggle: screen-lock (pixels) vs world units
        cb_lock = QCheckBox("Screen-locked spacing")
        cb_lock.setChecked(self._sketch_grid_screen_lock)
        cb_lock.stateChanged.connect(
            lambda _v: setattr(self, "_sketch_grid_screen_lock", cb_lock.isChecked())
            or self.view.update()
        )
        sp_grid = QDoubleSpinBox()
        sp_grid.setRange(0.1, 1000.0)
        sp_grid.setDecimals(2)
        sp_grid.setSingleStep(0.5)
        sp_grid.setValue(self._sketch_grid_step)
        sp_grid.valueChanged.connect(lambda v: setattr(self, "_sketch_grid_step", float(v)))
        hg.addWidget(cb_grid)
        hg.addWidget(cb_snapg)
        hg.addWidget(cb_lock)
        hg.addWidget(QLabel("Step"))
        hg.addWidget(sp_grid)
        fs.addRow("Grid/Snap", row_grid)
        # Kernel Sync (optional)
        row_sync = QWidget()
        hsync = QHBoxLayout(row_sync)
        hsync.setContentsMargins(0, 0, 0, 0)
        self._sketch_circle_to_ring = False
        cb_circle_sync = QCheckBox("Circle â†’ Ring (kernel)")
        cb_circle_sync.setToolTip(
            "When enabled, finalizing a sketch circle adds a thin torus at the same center/radius in the AACore scene."
        )
        cb_circle_sync.stateChanged.connect(
            lambda _v: setattr(self, "_sketch_circle_to_ring", cb_circle_sync.isChecked())
        )
        hsync.addWidget(cb_circle_sync)
        fs.addRow("Sync", row_sync)
        row_tools = QWidget()
        ht = QHBoxLayout(row_tools)
        ht.setContentsMargins(0, 0, 0, 0)
        btn_poly = QPushButton("Polyline")
        btn_poly.setCheckable(True)
        btn_line = QPushButton("Line")
        btn_line.setCheckable(True)
        btn_arc3 = QPushButton("Arc (3-pt)")
        btn_arc3.setCheckable(True)
        btn_circle = QPushButton("Circle")
        btn_circle.setCheckable(True)
        btn_rect = QPushButton("Rectangle")
        btn_rect.setCheckable(True)
        btn_insv = QPushButton("Insert Vertex")
        btn_insv.setCheckable(True)
        ...
        def _choose_poly():
            ...
            if on:
                for b2 in (btn_line, btn_arc3, btn_circle, btn_rect):
                    b2.setChecked(False)
        ...
        def _choose_line():
            ...
            if on:
                for b2 in (btn_poly, btn_arc3, btn_circle, btn_rect):
                    b2.setChecked(False)
        ...
        def _choose_arc3():
            ...
            if on:
                for b2 in (btn_poly, btn_line, btn_circle, btn_rect):
                    b2.setChecked(False)
        ...
        def _choose_circle():
            ...
            if on:
                for b2 in (btn_poly, btn_line, btn_arc3, btn_rect):
                    b2.setChecked(False)
        ...
        def _choose_rect():
            ...
            if on:
                for b2 in (btn_poly, btn_line, btn_arc3, btn_circle):
                    b2.setChecked(False)
        ...
        cb_ins_mid.stateChanged.connect(
            lambda _v: setattr(self, "_insert_midpoint", cb_ins_mid.isChecked())
        )
        ...
        def _choose_insv():
            ...
            if on:
                for b2 in (btn_poly, btn_line, btn_arc3, btn_circle, btn_rect):
                    b2.setChecked(False)
                self._sketch_mode = "insert_vertex"
                self._sketch._dim_pick = None
            else:
                if self._sketch_mode == "insert_vertex":
                    self._sketch_mode = None
            self.view.update()

        btn_insv.clicked.connect(_choose_insv)
        # Dimension tool
        btn_dim = QPushButton("Dimension")
        btn_dim.setCheckable(True)
        ...
        def _choose_dim():
            ...
            if on:
                ...
                if self._sketch_mode == "dimension":
                    ...
                    try:
                        self._sketch._dim_placing = False
                        self._sketch._dim_place_data = None
                        self._sketch._dim_preview = None
                    except Exception:
                        pass
            self.view.update()
        ...
        row_dim = QWidget()
        hdim = QHBoxLayout(row_dim)
        hdim.setContentsMargins(0, 0, 0, 0)
        from PySide6.QtWidgets import QComboBox as _QCB

        self._dim_type = "linear"
        cb_dim_type = _QCB()
        cb_dim_type.addItems(["Linear", "Horizontal", "Vertical", "Radius", "Diameter"])

        def _set_dim_type(idx):
            self._dim_type = ["linear", "horizontal", "vertical", "radius", "diameter"][
                max(0, min(4, idx))
            ]

        cb_dim_type.currentIndexChanged.connect(_set_dim_type)
        self._dim_precision = 2
        sp_dim_prec = QSpinBox()
        sp_dim_prec.setRange(0, 6)
        sp_dim_prec.setValue(self._dim_precision)
        sp_dim_prec.valueChanged.connect(lambda v: setattr(self, "_dim_precision", int(v)))
        hdim.addWidget(QLabel("Type"))
        hdim.addWidget(cb_dim_type)
        hdim.addWidget(QLabel("Precision"))
        hdim.addWidget(sp_dim_prec)
        ...
        row_io = QWidget()
        hio = QHBoxLayout(row_io)
        hio.setContentsMargins(0, 0, 0, 0)
        ...
        def _clear_sk():
            ...
            self._sketch.active_poly = None
            self._sketch.active_shape = None
            self.view._sk_point_sel = None
            self.view._sk_dragging = False
            self._update_sketch_info(None)
            self.view.update()

        btn_clear_sk.clicked.connect(_clear_sk)
        hio.addWidget(btn_save_sk)
        hio.addWidget(btn_load_sk)
        ...
        va = QVBoxLayout()
        gb_act.setLayout(va)
        btn_save = QPushButton("Save G-Buffers [G]")
        btn_save.setEnabled(False)
        btn_help = QPushButton("Print Shortcuts [H]")
        btn_help.clicked.connect(lambda: log.info(self.view._shortcuts_help()))
        ...
        def _do_showcase():
            try:
                ...
                if len(self.view.scene.prims) < MAX_PRIMS - 6:
                    self.view.scene.add(
                        Prim(KIND_SPHERE, [0.7, 0, 0, 0], beta=0.08, color=(0.95, 0.45, 0.35))
                    )
                    self.view.scene.add(
                        Prim(KIND_TORUS, [1.0, 0.22, 0, 0], beta=0.03, color=(0.6, 0.85, 0.5))
                    )
                    self.view.scene.add(
                        Prim(KIND_MOBIUS, [1.4, 0.25, 0, 0], beta=0.02, color=(0.7, 0.55, 0.95))
                    )
                    self.view.scene.add(
                        Prim(
                            KIND_SUPERELLIPSOID, [1.1, 3.5, 0, 0], beta=0.0, color=(0.95, 0.9, 0.85)
                        )
                    )
                    self.view.scene.add(
                        Prim(KIND_GYROID, [2.2, 0.0, 0.035, 0], beta=0.0, color=(0.65, 0.9, 0.3))
                    )
                    self.view.scene.add(
                        Prim(KIND_TREFOIL, [0.8, 0.08, 96, 0], beta=0.0, color=(0.95, 0.6, 0.25))
                    )
                    # Offset for variety
                    try:
                        for i, pr in enumerate(self.view.scene.prims[-6:]):
                            pr.xform.M[:3, 3] += np.array(
                                [(i % 3 - 1) * 1.6, (i // 3 - 0.5) * 1.4, 0.0], np.float32
                            )
                    except Exception:
                        pass
                    try:
        ...
        def _do_clear_scene():
            try:
                ...
                try:
                    ...
                    pass
                try:
                    self.view.clear_polyline_overlays()
                except Exception:
                    pass
                self._refresh_prim_label()
                self.view.update()
        ...
        va.addWidget(btn_save)
        va.addWidget(btn_help)
        va.addWidget(btn_showcase)
        va.addWidget(btn_clear_scene)
        side.addWidget(gb_act)
        self._register_section("actions", "Actions", gb_act)

        # --- Environment ---
        gb_env = QGroupBox("Environment")
        fe = QFormLayout()
        gb_env.setLayout(fe)
        self._bg_sliders = []
        for i, ch in enumerate(["R", "G", "B"]):
            ...
            s.setRange(0, 255)
            s.setValue(int(self.view.scene.bg_color[i] * 255))
            s.valueChanged.connect(self._on_bg_changed)
            fe.addRow(f"BG {ch}", s)
            self._bg_sliders.append(s)
        self._env_sliders = []
        for i, ch in enumerate(["X", "Y", "Z"]):
            s = QSlider(Qt.Orientation.Horizontal)
            s.setRange(-150, 150)
            s.setValue(int(self.view.scene.env_light[i] * 100))
            s.valueChanged.connect(self._on_env_changed)
            fe.addRow(f"Light {ch}", s)
            self._env_sliders.append(s)
        side.addWidget(gb_env)
        self._register_section("environment", "Environment", gb_env)

        # --- Primitives + Editing ---
        gb_prims = QGroupBox("Primitives")
        vp = QVBoxLayout()
        gb_prims.setLayout(vp)
        self._prim_list_label = QLabel("(0)")
        self._prim_buttons_box = QVBoxLayout()
        btn_add_sphere = QPushButton("Add Sphere")
        btn_add_sphere.clicked.connect(lambda: self._add_prim("sphere"))
        btn_add_box = QPushButton("Add Box")
        btn_add_box.clicked.connect(lambda: self._add_prim("box"))
        btn_add_capsule = QPushButton("Add Capsule")
        btn_add_capsule.clicked.connect(lambda: self._add_prim("capsule"))
        btn_add_torus = QPushButton("Add Torus")
        btn_add_torus.clicked.connect(lambda: self._add_prim("torus"))
        btn_add_mobius = QPushButton("Add Mobius")
        btn_add_mobius.clicked.connect(lambda: self._add_prim("mobius"))
        btn_add_superell = QPushButton("Add Superellipsoid")
        btn_add_superell.clicked.connect(lambda: self._add_prim("superellipsoid"))
        btn_add_qc = QPushButton("Add QuasiCrystal")
        btn_add_qc.clicked.connect(lambda: self._add_prim("quasicrystal"))
        btn_add_torus4d = QPushButton("Add 4D Torus")
        btn_add_torus4d.clicked.connect(lambda: self._add_prim("torus4d"))
        btn_add_mandelbulb = QPushButton("Add Mandelbulb")
        btn_add_mandelbulb.clicked.connect(lambda: self._add_prim("mandelbulb"))
        btn_add_klein = QPushButton("Add Klein Bottle")
        btn_add_klein.clicked.connect(lambda: self._add_prim("klein"))
        btn_add_menger = QPushButton("Add Menger Sponge")
        btn_add_menger.clicked.connect(lambda: self._add_prim("menger"))
        btn_add_hyperbolic = QPushButton("Add Hyperbolic Tiling")
        btn_add_hyperbolic.clicked.connect(lambda: self._add_prim("hyperbolic"))
        btn_add_gyroid = QPushButton("Add Gyroid")
        btn_add_gyroid.clicked.connect(lambda: self._add_prim("gyroid"))
        btn_add_trefoil = QPushButton("Add Trefoil Knot")
        btn_add_trefoil.clicked.connect(lambda: self._add_prim("trefoil"))
        btn_del_last = QPushButton("Delete Last")
        btn_del_last.clicked.connect(self._del_last)
        btn_dup_sel = QPushButton("Duplicate Selected")
        btn_dup_sel.clicked.connect(self._duplicate_selected)
        btn_center_sel = QPushButton("Center On Selected")
        btn_center_sel.clicked.connect(self._center_on_selected)
        btn_reset_cam = QPushButton("Reset Camera")
        btn_reset_cam.clicked.connect(self._reset_camera)
        btn_export = QPushButton("Export Scene JSON")
        btn_export.clicked.connect(self._export_scene_json)
        for b in (
            self._prim_list_label,
            btn_add_sphere,
            btn_add_box,
            btn_add_capsule,
            btn_add_torus,
        ...
        for b in (btn_dup_sel, btn_center_sel, btn_reset_cam, btn_export):
            vp.addWidget(b)
        vb_holder = QWidget()
        vb_holder.setLayout(self._prim_buttons_box)
        vp.addWidget(vb_holder)
        # Edit group
        self._edit_group = QGroupBox("Edit Selected")
        fe2 = QFormLayout()
        self._edit_group.setLayout(fe2)
        ...
        self._op_box = _QCB2()
        self._op_box.addItems(["solid", "subtract"])
        ...
        self._nudge_step.setRange(0.001, 100.0)
        self._nudge_step.setDecimals(3)
        self._nudge_step.setSingleStep(0.1)
        self._nudge_step.setValue(1.0)
        fe2.addRow("Nudge Step", self._nudge_step)
        self._edit_group.setEnabled(False)
        vp.addWidget(self._edit_group)
        for sp in self._sp_pos:
            sp.valueChanged.connect(self._apply_edit)
        for sp in self._sp_param:
            sp.valueChanged.connect(self._apply_edit)
        self._sp_beta.valueChanged.connect(self._apply_edit)
        self._op_box.currentIndexChanged.connect(self._apply_edit)
        ...
        self._current_sel = -1
        self._current_color = (0.8, 0.7, 0.6)
        side.addWidget(gb_prims)
        self._register_section("primitives", "Primitives", gb_prims)

        # --- Tesseract (4D hypercube) ---
        gb_tess = QGroupBox("Tesseract (4D Hypercube)")
        ft = QFormLayout()
        gb_tess.setLayout(ft)
        self._tess_edge = QDoubleSpinBox()
        self._tess_edge.setRange(0.1, 10.0)
        self._tess_edge.setDecimals(3)
        self._tess_edge.setSingleStep(0.1)
        self._tess_edge.setValue(2.0)
        self._tess_edge_rad = QDoubleSpinBox()
        self._tess_edge_rad.setRange(0.001, 1.0)
        self._tess_edge_rad.setDecimals(3)
        self._tess_edge_rad.setSingleStep(0.01)
        self._tess_edge_rad.setValue(0.03)
        self._tess_node_rad = QDoubleSpinBox()
        self._tess_node_rad.setRange(0.001, 1.0)
        self._tess_node_rad.setDecimals(3)
        self._tess_node_rad.setSingleStep(0.01)
        self._tess_node_rad.setValue(0.04)
        self._tess_include_nodes = QCheckBox("Include Nodes (spheres)")
        self._tess_include_nodes.setChecked(False)
        # Projection
        self._tess_proj = QComboBox()
        self._tess_proj.addItems(["Orthographic", "Perspective"])
        self._tess_fw = QDoubleSpinBox()
        self._tess_fw.setRange(0.5, 10.0)
        self._tess_fw.setDecimals(2)
        self._tess_fw.setSingleStep(0.1)
        self._tess_fw.setValue(3.0)
        ...
        self._tess_ang_xy = r4spin()
        self._tess_ang_xz = r4spin()
        self._tess_ang_xw = r4spin()
        self._tess_ang_yz = r4spin()
        self._tess_ang_yw = r4spin()
        self._tess_ang_zw = r4spin()
        self._tess_autospin = QCheckBox("Auto-Spin")
        self._tess_autospin.setChecked(True)
        self._tess_speed = QDoubleSpinBox()
        self._tess_speed.setRange(0.0, 360.0)
        self._tess_speed.setDecimals(2)
        self._tess_speed.setSingleStep(5.0)
        self._tess_speed.setValue(30.0)
        ...
        self._rig_timer.timeout.connect(self._update_rigs)
        self._rig_timer.start(33)

        # --- Pixel-to-Quantum Sphere ---
        gb_scale = QGroupBox("Pixel-to-Quantum Sphere")
        fs = QFormLayout()
        gb_scale.setLayout(fs)
        self._scale_pixels = QDoubleSpinBox()
        self._scale_pixels.setRange(0.5, 10.0)
        self._scale_pixels.setDecimals(2)
        self._scale_pixels.setSingleStep(0.25)
        self._scale_pixels.setValue(2.5)
        self._scale_pixels.setSuffix(" px")
        self._scale_duration = QDoubleSpinBox()
        self._scale_duration.setRange(2.0, 120.0)
        self._scale_duration.setDecimals(1)
        self._scale_duration.setSingleStep(0.5)
        self._scale_duration.setValue(12.0)
        self._scale_duration.setSuffix(" s")
        self._scale_fov = QDoubleSpinBox()
        self._scale_fov.setRange(20.0, 120.0)
        self._scale_fov.setDecimals(1)
        self._scale_fov.setSingleStep(1.0)
        self._scale_fov.setValue(60.0)
        self._scale_fov.setSuffix(" deg")
        self._scale_status = QLabel("Press start to generate the scale-aware sphere.")
        self._scale_status.setWordWrap(True)
        btn_start_scale = QPushButton("Start Scale Journey")
        btn_start_scale.clicked.connect(self._start_scale_sphere_journey)
        btn_clear_scale = QToolButton()
        btn_clear_scale.setText("Reset")
        btn_clear_scale.setToolTip("Pause the journey and tuck the sphere detail away.")
        btn_clear_scale.clicked.connect(self._stop_scale_sphere)
        ...
        # --- Molecules ---
        gb_mol = QGroupBox("Molecules")
        fmol = QFormLayout()
        gb_mol.setLayout(fmol)
        self._mol_atom_scale = QDoubleSpinBox()
        self._mol_atom_scale.setRange(0.1, 5.0)
        self._mol_atom_scale.setSingleStep(0.1)
        self._mol_atom_scale.setValue(0.6)
        self._mol_bond_r = QDoubleSpinBox()
        self._mol_bond_r.setRange(0.005, 1.0)
        self._mol_bond_r.setDecimals(3)
        self._mol_bond_r.setSingleStep(0.005)
        self._mol_bond_r.setValue(0.035)
        self._mol_bond_thresh = QDoubleSpinBox()
        self._mol_bond_thresh.setRange(0.0, 1.0)
        self._mol_bond_thresh.setDecimals(3)
        self._mol_bond_thresh.setSingleStep(0.02)
        self._mol_bond_thresh.setValue(0.2)
        self._mol_autospin = QCheckBox("Auto-Spin")
        self._mol_autospin.setChecked(True)
        self._mol_speed = QDoubleSpinBox()
        self._mol_speed.setRange(0.0, 360.0)
        self._mol_speed.setDecimals(1)
        self._mol_speed.setSingleStep(5.0)
        self._mol_speed.setValue(25.0)
        ...
        if not have_ai:
            if self._ai_transcript is not None:
                try:
                    self._ai_transcript.append(
                        "AI copilot unavailable. Install 'openai' and set OPENAI_API_KEY."
                    )
                except Exception:
                    pass
            ...
            if self._ai_input is not None:
                ...
                self._ai_input.setPlaceholderText(
                    "AI copilot unavailable (missing dependencies or API key)."
                )
    ...
    def _ai_create_circle(
        self, radius: float, cx: float | None = None, cy: float | None = None, segments: int = 256
    ) -> dict:
        try:
    ...
    def _ai_create_line(self, x1: float, y1: float, x2: float, y2: float) -> dict:
        ...
        return {
            "ok": True,
            "line": {"start": [pts[0][0], pts[0][1]], "end": [pts[1][0], pts[1][1]]},
        }
    ...
    def _rebuild_wheel_tools(self) -> None:
        ...
        base = [
            (
                ...
                else ToolSpec(spec.label, spec.tool_id, list(getattr(spec, "children", [])))
            )
    ...
    def _add_prim(self, kind: str):
        ...
        if kind == "sphere":
            ...
            self.view.scene.add(
                Prim(KIND_CAPSULE, [0.3, 1.5, 0, 0], beta=0.02, color=(0.6, 0.9, 0.5))
            )
        elif kind == "torus":
            self.view.scene.add(
                Prim(KIND_TORUS, [0.9, 0.25, 0, 0], beta=0.03, color=(0.8, 0.6, 0.4))
            )
        elif kind == "mobius":
            # params: [R, width]
            self.view.scene.add(
                Prim(KIND_MOBIUS, [1.2, 0.25, 0, 0], beta=0.02, color=(0.7, 0.5, 0.9))
            )
        elif kind == "superellipsoid":
            # params: [radius, power]
            self.view.scene.add(
                Prim(KIND_SUPERELLIPSOID, [1.2, 4.0, 0, 0], beta=0.0, color=(0.95, 0.9, 0.85))
            )
        elif kind == "quasicrystal":
            # params: [scale, iso, thickness]
            self.view.scene.add(
                Prim(KIND_QUASICRYSTAL, [3.0, 1.0, 0.02, 0], beta=0.0, color=(0.2, 0.9, 0.9))
            )
        elif kind == "torus4d":
            # params: [R1, R2, r, w_slice] - two major radii, minor radius, 4D slice position
            self.view.scene.add(
                Prim(KIND_TORUS4D, [1.0, 0.8, 0.3, 0.0], beta=0.0, color=(0.9, 0.3, 0.9))
            )
        elif kind == "mandelbulb":
            # params: [power, bailout, max_iter, scale] - fractal parameters (better visibility)
            self.view.scene.add(
                Prim(KIND_MANDELBULB, [8.0, 2.0, 24, 0.8], beta=0.0, color=(0.9, 0.2, 0.1))
            )
        elif kind == "klein":
            # params: [scale, n, t_offset, thickness] - Klein bottle parameters (75% smaller)
            self.view.scene.add(
                Prim(KIND_KLEIN, [0.25, 2.0, 0.0, 0.025], beta=0.0, color=(0.1, 0.7, 0.9))
            )
        elif kind == "menger":
            ...
            self.view.scene.add(
                Prim(KIND_HYPERBOLIC, [2.0, 7, 3, 0], beta=0.0, color=(0.9, 0.3, 0.7))
            )
        elif kind == "gyroid":
            # params: [scale, tau, thickness, 0]
            self.view.scene.add(
                Prim(KIND_GYROID, [2.5, 0.0, 0.04, 0], beta=0.0, color=(0.7, 0.9, 0.3))
            )
        elif kind == "trefoil":
            # params: [scale, tube, samples, 0]
            self.view.scene.add(
                Prim(KIND_TREFOIL, [0.8, 0.08, 96, 0], beta=0.0, color=(0.9, 0.5, 0.2))
            )
        self._refresh_prim_label()
        self.view.update()
    ...
    def _kind_name(self, k):
        return {
            ...
            KIND_TORUS4D: "4D Torus",
            KIND_MANDELBULB: "Mandelbulb",
            KIND_KLEIN: "Klein Bottle",
            KIND_MENGER: "Menger Sponge",
            KIND_HYPERBOLIC: "Hyperbolic Tiling",
            KIND_GYROID: "Gyroid",
            KIND_TREFOIL: "Trefoil Knot",
        }.get(k, str(k))
    ...
    def _select_prim(self, idx: int):
        ...
        pos = pr.xform.M[:3, 3]
        for i in range(3):
            self._sp_pos[i].blockSignals(True)
            self._sp_pos[i].setValue(float(pos[i]))
            self._sp_pos[i].blockSignals(False)
        ...
        self._sp_param[0].setValue(float(pr.params[0]))
        self._sp_param[0].blockSignals(False)
        self._sp_param[1].blockSignals(True)
        self._sp_param[1].setValue(float(pr.params[1] if pr.kind != KIND_SPHERE else 0.0))
        self._sp_param[1].blockSignals(False)
        self._sp_beta.blockSignals(True)
        self._sp_beta.setValue(pr.beta)
        self._sp_beta.blockSignals(False)
        self._op_box.blockSignals(True)
        self._op_box.setCurrentIndex(0 if pr.op == "solid" else 1)
        self._op_box.blockSignals(False)
        self._current_color = tuple(pr.color[:3])
        # rotation / scale sync
        if hasattr(pr, "euler"):
            for i in range(3):
                self._sp_rot[i].blockSignals(True)
                self._sp_rot[i].setValue(float(pr.euler[i]))
                self._sp_rot[i].blockSignals(False)
        if hasattr(pr, "scale"):
            for i in range(3):
                self._sp_scl[i].blockSignals(True)
                self._sp_scl[i].setValue(float(pr.scale[i]))
                self._sp_scl[i].blockSignals(False)
            try:
        ...
        try:
            ...
            if kind_name == "Sphere":
                ...
                a_tt = "Ring Radius R"
                b_tt = "Strip Half-Width"
            elif kind_name == "Box":
                a_tt = "Half-Extent X"
                b_tt = "Half-Extent Y (Z via scale)"
            elif kind_name == "Superellipsoid":
                a_tt = "Radius (overall)"
                b_tt = "Power p (boxier > 2)"
            elif kind_name == "QuasiCrystal":
                a_tt = "Scale (frequency)"
                b_tt = "Iso (level)"
            elif kind_name == "4D Torus":
                a_tt = "Major Radius 1 (XY plane)"
                b_tt = "Major Radius 2 (ZW plane)"
            elif kind_name == "Mandelbulb":
                a_tt = "Power (8.0 = classic)"
                b_tt = "Bailout Radius (escape)"
            elif kind_name == "Klein Bottle":
                a_tt = "Scale (overall size)"
                b_tt = "Shape Parameter"
            elif kind_name == "Menger Sponge":
                a_tt = "Iterations (detail level)"
                b_tt = "Size (overall scale)"
            elif kind_name == "Hyperbolic Tiling":
                a_tt = "Scale (overall size)"
                b_tt = "Order (polygon sides)"
            elif kind_name == "Gyroid":
                a_tt = "Scale (frequency)"
                b_tt = "Tau (iso-level)"
            elif kind_name == "Trefoil Knot":
                a_tt = "Scale (curve size)"
                b_tt = "Tube Radius"
            else:
                a_tt = "Param A"
                b_tt = "Param B"
    ...
    def _save_sketch_json(self):
        try:
            ...
            path, _ = QFileDialog.getSaveFileName(
                self, "Save Sketch JSON", "sketch.json", "JSON (*.json)"
            )
            if not path:
                return
            ...
            extra = {
                ...
                "arcs3": [],
                "dimensions": [],
                "dim_precision": int(getattr(self, "_dim_precision", 2)),
            }
            for ent in self._sketch.entities:
                if isinstance(ent, Polyline2D):
                    ...
                    for pt in ent.pts:
                        ...
                        doc.points[-1].__dict__.update(
                            {"id": idp, "x": float(pt[0]), "y": float(pt[1]), "construction": False}
                        )
                    doc.polylines.append(type("PL", (), {})())
                    doc.polylines[-1].__dict__.update(
                        {"id": f"pl{pid}", "verts": ids, "closed": False, "construction": False}
                    )
                elif isinstance(ent, Circle2D):
                    idc = new_pid()
                    c = ent.center
                    ...
                    doc.points[-1].__dict__.update(
                        {"id": idc, "x": float(c[0]), "y": float(c[1]), "construction": False}
                    )
                    doc.circles.append(type("C", (), {})())
                    doc.circles[-1].__dict__.update(
                        {
                            "id": f"c{pid}",
                            "center": idc,
                    ...
                    if (
                        getattr(ent, "p0", None) is not None
                        and getattr(ent, "p1", None) is not None
                        and getattr(ent, "p2", None) is not None
                    ):
                        extra["arcs3"].append(
                            {
                                "p0": [float(ent.p0[0]), float(ent.p0[1])],
                                "p1": [float(ent.p1[0]), float(ent.p1[1])],
                                "p2": [float(ent.p2[0]), float(ent.p2[1])],
                            }
                        )
            # dimensions
            for d in getattr(self._sketch, "dimensions", []) or []:
                try:
                    if (
                        isinstance(d, DimDiameter2D)
                        and isinstance(d.circle_ref, tuple)
                        and d.circle_ref[0] == "circle"
                        and isinstance(d.circle_ref[1], Circle2D)
                    ):
                        c = d.circle_ref[1]
                        extra["dimensions"].append(
                            {"type": "diameter", "center": [float(c.center[0]), float(c.center[1])]}
                        )
                        ...
                        if a is not None and b is not None:
                            ...
                            t = "linear"
                            if isinstance(d, DimHorizontal2D):
                                t = "horizontal"
                            elif isinstance(d, DimVertical2D):
                                t = "vertical"
                            extra["dimensions"].append(
                                {
                                    "type": t,
                                    ...
                                    "offset": float(getattr(d, "offset", 0.0)),
                                }
                            )
                except Exception:
                    pass
    ...
    def _load_sketch_json(self):
        try:
            ...
            for li in raw.get("lines", []) or []:
                p0 = np.array(
                    [float(li.get("p0", [0, 0])[0]), float(li.get("p0", [0, 0])[1])], np.float32
                )
                p1 = np.array(
                    [float(li.get("p1", [0, 0])[0]), float(li.get("p1", [0, 0])[1])], np.float32
                )
                self._sketch.entities.append(Line2D(p0, p1))
            for r in raw.get("rects", []) or []:
                p0 = np.array(
                    [float(r.get("p0", [0, 0])[0]), float(r.get("p0", [0, 0])[1])], np.float32
                )
                p1 = np.array(
                    [float(r.get("p1", [0, 0])[0]), float(r.get("p1", [0, 0])[1])], np.float32
                )
                self._sketch.entities.append(Rect2D(p0, p1))
            for a in raw.get("arcs3", []) or []:
                p0 = np.array(
                    [float(a.get("p0", [0, 0])[0]), float(a.get("p0", [0, 0])[1])], np.float32
                )
                p1 = np.array(
                    [float(a.get("p1", [0, 0])[0]), float(a.get("p1", [0, 0])[1])], np.float32
                )
                p2 = np.array(
                    [float(a.get("p2", [0, 0])[0]), float(a.get("p2", [0, 0])[1])], np.float32
                )
            ...
            for d in raw.get("dimensions", []) or []:
                ...
                if t == "diameter":
                    ...
                    for ent in self._sketch.entities:
                        if (
                            isinstance(ent, Circle2D)
                            and abs(ent.center[0] - cx) < 1e-6
                            and abs(ent.center[1] - cy) < 1e-6
                        ):
                            circ = ent
                            break
                    if circ is not None:
                    ...
                    if a is not None and b is not None:
                        ...
                        if t == "horizontal":
                            self._sketch.dimensions.append(
                                DimHorizontal2D(pa, pb, self, offset=off)
                            )
                        elif t == "vertical":
            ...
            try:
                self._dim_precision = int(
                    raw.get("dim_precision", getattr(self, "_dim_precision", 2))
                )
            except Exception:
                pass
            self.view.update()
    ...
    def viewport(self) -> AnalyticViewport:
        return self.view
    ...
    try:
        ...
        if panel is not None:
            setattr(
                panel,
                "_dim_precision",
                int(data.get("dim_precision", getattr(panel, "_dim_precision", 2))),
            )
            setattr(
                panel, "_dim_type", str(data.get("dim_type", getattr(panel, "_dim_type", "linear")))
            )
            if hasattr(panel, "_wheel_enabled"):
                ...
                panel._wheel_edge_overshoot = int(
                    data.get("wheel_overshoot_px", panel._wheel_edge_overshoot)
                )
                panel._wheel_thickness = float(
                    _clamp(data.get("wheel_thickness_ratio", panel._wheel_thickness), 0.02, 0.40)
                )
                panel._wheel_opacity = float(
                    _clamp(data.get("wheel_opacity", panel._wheel_opacity), 0.0, 1.0)
                )
                panel._wheel_text_scale = float(
                    _clamp(data.get("wheel_text_scale", panel._wheel_text_scale), 0.5, 2.5)
                )
                panel._wheel_auto_spin = bool(data.get("wheel_auto_spin", panel._wheel_auto_spin))
                panel._wheel_auto_spin_speed = float(
                    _clamp(
                        data.get("wheel_auto_spin_speed", panel._wheel_auto_spin_speed), 0.0, 360.0
                    )
                )
        ...
        try:
            ...
            if panel is not None and sk is not None and hasattr(panel, "_sketch"):
                ...
                for c in sk.get("circles", []) or []:
                    ctr = np.array(
                        [float(c.get("center", [0, 0])[0]), float(c.get("center", [0, 0])[1])],
                        np.float32,
                    )
                ...
                try:
                    ...
                    for d in sk.get("dimensions", []) or []:
                        ...
                        if t == "diameter":
                            ...
                            if circ is not None:
                                panel._sketch.dimensions.append(
                                    DimDiameter2D(("circle", circ), panel)
                                )
                        else:
                            a = d.get("a")
                            b = d.get("b")
                            if a is not None and b is not None:
                                ...
                                if t == "horizontal":
                                    panel._sketch.dimensions.append(
                                        DimHorizontal2D(pa, pb, panel, offset=off)
                                    )
                                elif t == "vertical":
                                    panel._sketch.dimensions.append(
                                        DimVertical2D(pa, pb, panel, offset=off)
                                    )
                                else:
                                    panel._sketch.dimensions.append(
                                        DimLinear2D(pa, pb, panel, offset=off)
                                    )
                except Exception:
                    pass
                # grid settings
                try:
                    ...
                    panel._sketch_grid_screen_lock = bool(
                        sk.get("grid_screen_lock", getattr(panel, "_sketch_grid_screen_lock", True))
                    )
                except Exception:
                    pass
        except Exception:
...
def _save_settings(self):
    try:
        ...
        if (
            panel is not None
            and hasattr(panel, "_section_vis")
            and isinstance(panel._section_vis, dict)
        ):
            data_update["section_visibility"] = {
                str(k): bool(v) for k, v in panel._section_vis.items()
            }
        ...
        try:
            ...
            if panel is not None and hasattr(panel, "_sketch"):
                sk = {
                    ...
                    "arcs3": [],
                    "dimensions": [],
                    "grid_on": bool(getattr(panel, "_sketch_grid_on", False)),
                    "snap_grid": bool(getattr(panel, "_sketch_snap_grid", False)),
                    "grid_step": float(getattr(panel, "_sketch_grid_step", 10.0)),
                    "grid_screen_lock": bool(getattr(panel, "_sketch_grid_screen_lock", True)),
                }
                for ent in getattr(panel, "_sketch").entities:
                    if isinstance(ent, Polyline2D):
