    ...
    def from_analytic_primitive(cls, primitive):
        ...
        if hasattr(primitive, "radius"):
            if hasattr(primitive, "height"):  # Cylinder or Capsule
                params = np.array(
                    [*primitive.position, primitive.radius, primitive.height], dtype=np.float32
                )
            elif hasattr(primitive, "minor_radius"):  # Torus
                params = np.array(
                    [*primitive.position, primitive.major_radius, primitive.minor_radius],
                    dtype=np.float32,
                )
            else:  # Sphere
...
class Scene:
    ...
    def to_gpu_structs(self):
        ...
        kind_map = {"sphere": 0, "box": 1, "cylinder": 2, "capsule": 3, "torus": 4}
        ...
        return dict(
            n=n, kinds=kinds, params=params, colors=colors, betas=betas, ids=ids, xforms=xforms
        )
