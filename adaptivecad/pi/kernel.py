...
def adaptive_arc_length(radius: float, angle_rad: float, kappa: float, scale: float,
                        params: PiAParams = PiAParams()) -> float:
...
def make_adaptive_circle(radius: float, n: int = 128, kappa: float = 0.0, scale: float = 1.0,
                         params: PiAParams = PiAParams()) -> np.ndarray:
    """
