...
def main():
    ...
    for rho in args.rho:
        for seed in args.seeds:
            ...
            lam_alpha_corr = (
                float(np.corrcoef(lam_vals, alpha_vals)[0, 1]) if lam_vals.size > 1 else 0.0
            )
            ...
            plt.imshow(
                lam_plot, origin="lower", cmap="viridis", extent=(-args.R, args.R, -args.R, args.R)
            )
            ...
            plt.figure(figsize=(4, 4))
            plt.imshow(
                alpha_plot, origin="lower", cmap="plasma", extent=(-args.R, args.R, -args.R, args.R)
            )
