...
from adaptivecad.aacore.sdf import Scene, Prim, KIND_KLEIN
...
class KleinBottleRunner:
    ...
    def step(self):
        ...
        if _HAVE_PIL:
            ...
            draw.text((50, self.args.h - 120), f"4D Rotation: {rotation_degrees:.0f}°", 
                     fill=(255, 200, 100), font=font_small)
            
            # Impossible geometry note
            draw.text((50, self.args.h - 80), "Self-intersecting 4D surface", 
                     fill=(200, 255, 200), font=font_small)
            
            if t > 0.6:
                draw.text((50, self.args.h - 40), "Impossible in 3D space", 
