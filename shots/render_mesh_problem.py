...
from PySide6.QtGui import QPainter, QFont, QPen, QColor, QBrush
from OpenGL.GL import *

# Import project modules
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from adaptivecad.gui.analytic_viewport import AnalyticViewport
from adaptivecad.aacore.sdf import Scene, Prim, KIND_TORUS
            ...
            if zoom_factor > 2.0:  # When zoomed in, emphasize the problem
                ...
                if zoom_factor > 3.0:
                    ...
                    for pt in [p1, p2, p3, p4]:
                        draw.ellipse([pt[0]-dot_size, pt[1]-dot_size, 
                                    pt[0]+dot_size, pt[1]+dot_size], 
                                   fill=(200, 100, 100))
            else:
    ...
    def step(self):
        ...
        if _HAVE_PIL:
            ...
            if t < 0.25:
                ...
                draw.text((self.args.w//2 + 40, 40), "Adaptive π", fill=(120, 255, 180), font=font_small)
            elif t > 0.4:
                # Problem becomes visible earlier
                if t > 0.5:
                    draw.text((self.args.w//2 - 180, self.args.h - 140), "Every CAD system", 
                             fill=(255, 255, 255), font=font_large)
                    draw.text((self.args.w//2 - 140, self.args.h - 100), "has this problem.", 
                             fill=(255, 255, 255), font=font_large)
                if t > 0.75:
                    draw.text((self.args.w//2 - 80, self.args.h - 60), "Until now.", 
                             fill=(120, 255, 180), font=font_large)
                
                # Add adaptive feature explanations
                if debug_mode == 5 and t > 0.6:
                    draw.text((self.args.w//2 + 40, self.args.h - 100), "Adaptive Heatmap", 
                             fill=(255, 255, 100), font=font_small)
                    draw.text((self.args.w//2 + 40, self.args.h - 70), "Shows distance field", 
                             fill=(200, 200, 200), font=font_small)
                    draw.text((self.args.w//2 + 40, self.args.h - 40), "gradients", 
                             fill=(200, 200, 200), font=font_small)
                
                # Add zoom indicator when appropriate
                if t > 0.6:
                    zoom_text = f"{zoom_factor:.1f}x zoom"
                    draw.text((40, self.args.h - 60), zoom_text, 
                             fill=(255, 200, 100), font=font_small)
            
            # Divider line
            draw.line([(self.args.w//2, 0), (self.args.w//2, self.args.h)], 
