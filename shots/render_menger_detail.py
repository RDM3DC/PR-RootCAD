...
from adaptivecad.aacore.sdf import Scene, Prim, KIND_MENGER
...
class MengerZoomRunner:
    ...
    def step(self):
        ...
        if _HAVE_PIL:
            ...
            draw.text((50, self.args.h - 140), f"Detail Level: {detail_level}", 
                     fill=(150, 255, 150), font=font_small)
            
            # Zoom level
            draw.text((50, self.args.h - 100), f"Zoom: {zoom_factor:.1f}x", 
            ...
            draw.text((50, self.args.h - 60), ops_text, 
                     fill=(255, 180, 100), font=font_small)
            
            # Mathematical note
            if t > 0.7:
                draw.text((50, self.args.h - 20), "Perfect boolean operations", 
        ...
        if (self.i % 10) == 0:
            print(f"Rendered {self.i+1}/{self.nframes} (detail: L{detail_level}, zoom: {zoom_factor:.1f}x)")
        self.i += 1
