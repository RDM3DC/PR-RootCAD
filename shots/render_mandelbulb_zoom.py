...
from adaptivecad.aacore.sdf import Scene, Prim, KIND_MANDELBULB
...
class MandelbulbZoomRunner:
    ...
    def step(self):
        ...
        self.view.pitch = -0.1 + 0.2 * math.sin(2 * math.pi * t_raw * 0.7) + 0.05 * math.cos(t_raw * math.pi * 15)
        ...
        if _HAVE_PIL:
            ...
            draw.text((50, self.args.h - 80), f"Detail: {detail_level} iterations", 
                     fill=(200, 200, 255), font=font_small)
            
            # Mathematical note for high zoom
            if t_raw > 0.7:
                draw.text((50, self.args.h - 40), "Mathematically infinite detail", 
